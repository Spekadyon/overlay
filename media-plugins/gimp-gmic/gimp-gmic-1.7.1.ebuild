...
src_compile() {
	if ! use openmp ; then
		emake CC="$(tc-getCXX)" CFLAGS="${CXXFLAGS}" OPT_CFLAGS= DEBUG_CFLAGS= NOSTRIP="YES" NOOPENMP="YES" gimp
	else
		emake CC="$(tc-getCXX)" CFLAGS="${CXXFLAGS}" OPT_CFLAGS= DEBUG_CFLAGS= NOSTRIP="YES" gimp
	fi
